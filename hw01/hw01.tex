    ...
    \begin{enumerate}[label=\theenumi\Alph*]
        \item  For a positive-definite, symmetric, $m\times m$ matrix $G$, there exists a $\sqrt{G}$ such that $(\sqrt{G})^2 = G$.
            \begin{proof}
              ...
              positive-definite, $G$ has stricly positive eigenvalues, and therefore $D$ is a positive diagonal matrix. As a result,
        ...
        \item There exists an invertible linear transformation $f: \mathbb{R}^m\rightarrow \mathbb{R}^m$ such that $v\cdot_{G} w = f(v)\cdot f(w)$ for all $v,w \in \mathbb{R}^m$.
              \begin{proof}
                Let $f(v)=\sqrt{G}v$. We begin by showing that $f$ is invertible, by demonstrating that is bijective.\newline First, we show that $f$ is injective, and assume that $f(v)=f(w)$ for some $v,w \in {\mathbb{R}}^m$. By the definition of $f$, this is equivalent to $\sqrt{G}v=\sqrt{G}w$.
                We now show that $\sqrt{G}$ has an inverse, by demonstrating that $(\sqrt{G})^{-1}=P(\sqrt{D})^{-1}P^{-1}$ from problem B.A. Note that since $D$ is a positive diagonal matrix, $\sqrt{D}$ is as well, and is therefore invertible.
                ...
                $f$ to be bijective, and therefore invertible.\newline
